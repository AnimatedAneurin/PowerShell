...
function Remove-RegistryHKCRInstallerProducts {
    ...
    $deletedKeysHKCRInstallerProducts = @()
    ...
    foreach ($subKey in $subKeys) {
        ...
        if ($productName -and $productName -like "*Qualys*") {
            ...
            $deletedKeysHKCRInstallerProducts += $regkeyToDelete
    ...
    if (($deletedKeysHKCRInstallerProducts.count -eq 0) -and ($DeleteRegValue -eq $FALSE)) {
    ...
    return $deletedKeysHKCRInstallerProducts
}
...
# HEALTH CHECK (PART 2 / 4) - IF AGENT ALREADY EXISTS, BUT REGISTRY IS UNHEALTHY. CLEAN INSTALL #
        ...
        if ($anyKeyMissing) {
            ...
            LogWrite -logstring "[Information] Detected that QualysAgent.exe is already installed, but Registry is unhealthy and the Uninstaller is missing. Performing Deep Clean..."
            LogWrite -logstring "[Action] Cleansing Qualys from Services..."
            Remove-ServiceName -DeleteService $True
            LogWrite -logstring "[Action] Cleansing Qualys from Registry..."
            Remove-RegistryQualys -DeleteRegValue $True
